...
function getMappedPath(source, file, retry) {
  ...
  if (retry && paths.length === 0) {
    if (JS_EXT_PATTERN.test(source)) {
      ...
      return getMappedPath(basename + tsExt, file) || getMappedPath(source + "/index.ts", file) || getMappedPath(source + "/index.tsx", file) || getMappedPath(source + "/index.d.ts", file) || getMappedPath(basename + ".d" + (tsExt === ".tsx" ? ".ts" : tsExt), file) || getMappedPath(source + "/index.js", file);
    }
    return getMappedPath(source + ".ts", file) || getMappedPath(source + ".tsx", file) || getMappedPath(source + ".js", file) || getMappedPath(source + ".d.ts", file) || getMappedPath(source + "/index.ts", file) || getMappedPath(source + "/index.tsx", file) || getMappedPath(source + "/index.d.ts", file) || getMappedPath(source + "/index.js", file);
  }
  if (paths.length > 1) {
